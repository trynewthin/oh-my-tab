...
const resources: SiteResources = {
  ...
  privacy: {
    ...
    summary: {
      ...
      consent: {
        title: "联网需要主动开启",
        text: "联想、图标、WebDAV 及天气和 RSS 的手动请求都由你选择并授权。",
      },
      revoke: {
        title: "随时可以停止",
    ...
    sections: {
      "local-data": {
        title: "本地数据",
        paragraphs: [
          "网站标题和链接、文件夹、布局、顶栏的网页入口与操作配置、搜索引擎、偏好、点阵图片与植物进度保存在 Chrome 本地存储；开发预览使用 IndexedDB。背景原图和图标缓存由同一存储层管理。组件的便签、倒计时日期、时区、计时进度、手动天气坐标、订阅源地址和本地重新编码的组件图片也保存在本地。",
      ...
      "network-services": {
        ...
        paragraphs: [
          ...
          "天气和 RSS 组件不会自动加载。只有点击“加载/刷新”时才会申请精确 HTTPS 主机权限，并把所配置的坐标或订阅源地址直接发给相应服务；不会调用设备定位、使用代理或加载文章 HTML。订阅源查询参数会随地址保存在本地，请勿填写秘密。响应限制为 1 MiB，只保留在页面内存且不进入备份。",
      ...
      webdav: {
        ...
        paragraphs: [
          ...
          "上传的 ZIP 包含网站标题与链接、布局、设置、点阵、植物进度、教程状态、原始背景图片，以及组件设置、便签、计时进度、倒计时日期、时区、天气坐标、订阅源地址和本地重新编码的组件图片。服务器地址和用户名保存在当前设备；密码只在本次设置面板打开期间保留于内存，不写入本地备份或同步文件。",
      ...
      control: {
        ...
        paragraphs: [
          ...
          "ZIP 备份未加密，包含网站链接、设置、原始背景图片、便签、本地重新编码的组件图片、坐标和完整订阅源地址，请只交给可信接收者。隐私授权和 WebDAV 连接信息不会随备份导入。旧版 localStorage 数据只作为迁移回退副本保留，并在卸载扩展时一并删除。",
        ],
      },
      "use-and-sharing": {
        title: "数据用途与共享限制",
        paragraphs: [
          ...
          "使用第三方搜索、图标、WebDAV、天气或 RSS 服务时，数据由你选择的服务接收，其保留与删除方式由相应服务的政策决定。开发者无法代第三方承诺日志保留期限。",
        ],
      },
      contact: {
        title: "联系与政策更新",
