...
test("Pixi matrix renders at the existing dimensions and remains idle for static text", async ({
  page,
}, testInfo) => {
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      "omt.home-settings",
      JSON.stringify({
        state: { content: "text", text: "HELLO", topComponent: "dot-matrix" },
        version: 0,
      })
    )
  })
  ...
  const count = await matrix
    .locator("[data-pixi-matrix]")
    .getAttribute("data-render-count")
  await page.waitForTimeout(1200)
  await expect(matrix.locator("[data-pixi-matrix]")).toHaveAttribute(
    "data-render-count",
    count!
  )
...
test("Pixi clock updates and keeps an accessible time label", async ({
  page,
}) => {
  await page.addInitScript(() =>
    localStorage.setItem(
  ...
  await expect(matrix.locator("[data-pixi-matrix]")).not.toHaveAttribute(
    "data-render-count",
    "1"
  )
})
