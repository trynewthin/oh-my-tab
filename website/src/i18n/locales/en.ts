  ...
  privacy: {
    ...
    summary: {
      ...
      consent: {
        ...
        text: "Suggestions, favicons, WebDAV, and manual weather and RSS requests are chosen and authorised by you.",
      },
      revoke: {
    ...
    sections: {
      ...
      "network-services": {
        ...
        paragraphs: [
          ...
          "Weather and RSS widgets never load automatically. Only a Load or Refresh click requests the exact HTTPS host permission and sends the configured coordinates or feed URL directly to that service. No device geolocation, proxy, or article HTML is used. Feed query parameters are stored with the feed URL, so do not enter secrets. Responses are limited to 1 MiB, kept only in page memory, and are not backed up.",
      ...
      webdav: {
        ...
        paragraphs: [
          ...
          "An uploaded ZIP contains site titles and links, layout, settings, dot-matrix images, plant progress, tutorial state, original background images, widget settings, notes, timer progress, countdown dates, time zones, weather coordinates, feed URLs, and locally re-encoded widget photos. The server address and username are stored on this device; the password is kept in memory only while the settings panel is open and is never written to local backups or sync files.",
      ...
      "use-and-sharing": {
        ...
        paragraphs: [
          ...
          "When you use a third-party search, favicon, WebDAV, weather, or RSS service, data is received by the service you chose, and its retention and deletion practices are governed by that service's own policy. The developer cannot promise a third party's log retention period.",
        ],
      },
      contact: {
