  ...
  privacy: {
    ...
    sections: {
      "local-data": {
        title: "本地数据",
        paragraphs: [
          "网站标题和链接、文件夹、布局、顶栏的网页入口与操作配置、搜索引擎、偏好、点阵图片与植物进度保存在 Chrome 本地存储；开发预览使用 IndexedDB。背景原图和图标缓存由同一存储层管理。",
      ...
      "network-services": {
        ...
        paragraphs: [
          ...
          "网站图标下载默认关闭。启用并授权后，书签或顶栏的网页入口在图标缓存缺失或手动刷新时，会把网站域名发送给 Favicon.im，失败时发送给 DuckDuckGo。请求不包含网站名称、完整网址路径或查询参数。",
