...
- **组件与文件夹**：四宫格菜单或网格空白处右键添加组件。标签支持 4×1、4×2；文件夹支持 4×2、4×4、4×8，可拖入、拖出和内部排序。
...
- **快捷搜索**：匹配主页及文件夹内书签，接入 Bing 实时联想；结果使用当前选择的搜索引擎打开。支持自定义引擎和键盘选择。
- **导入与备份**：浏览器书签 HTML 增量导入、网址去重、同名文件夹合并；多级目录以路径名称保留。配置可压缩导出到剪贴板，导入前校验并确认覆盖。
- **本地存储**：书签、布局、偏好和网站图标缓存保存在本地。首次使用有分步引导，可随时从常规设置重新开启。
