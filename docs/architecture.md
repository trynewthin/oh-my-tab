...
`lib/garden.ts` 负责生长、点数结算和养护规则，`garden-plant-art.tsx` 根据持久化种子生成外形，编辑面板只组织操作。种子决定高度、倾斜、分枝、叶片、花色、花瓣排列、生长周期与动画节奏。相同种子在刷新及图鉴切换后保持一致。

自然生长周期为 72–120 小时，分为萌芽、幼苗、生长、花期、成熟五个阶段。初始 6 点，每小时积累 1 点（离线时间计入）；浇水花费 1 点增加 6 小时进度，施肥花费 2 点增加 18 小时进度。结算保存时间基准并保留不足一小时的余量，避免刷新或图鉴切换重复领取。成熟植物可收纳，当前植物未成熟时不能切换或重复播种。植物名称、随机种子、加速进度、点数与图鉴随网格配置持久化。

花盆的点数、计时基准、每日签到和图鉴统一保存在 `garden-store.ts`（`omt.garden`），网格组件只保存当前植物。初始化时合并旧花盆余额并按植物身份去重图鉴，迁移只执行一次。配置导出包含共享数据；导入旧配置时从其中的花盆数据构建共享状态。

## PixiJS 动画渲染

电子点阵使用共享圆角纹理与按需提交绘制。燃烧、粒子、文件夹光效和花盆使用 PixiJS；装饰区域共用 WebGL 渲染器，每个区域的 Canvas 负责呈现渲染结果，避免为每张卡片创建 WebGL 上下文。呈现阶段仍包含画布复制，其 GPU 成本需要在目标设备上实测。

花盆按动画分组缓存 SVG 部件纹理，保留整株、叶片和花朵的变换，源 SVG 仅用于生成素材与故障恢复。离屏和后台停止可见动画调度，组件卸载时释放场景、纹理与渲染器引用。PixiJS 使用静态函数的 CSP 适配，不修改扩展脚本安全策略。
